...
//                    debug: true, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
                ...
                wx.chooseWXPay({
                    ...
                    nonceStr: data.body.payData.noncestr, // 支付签名随机串，不长于 32 位
                    package: data.body.payData.packageStr, // 统一支付接口返回的prepay_id参数值，提交格式如：prepay_id=***）
                    signType: 'SHA1', // 签名方式，默认为'SHA1'，使用新版支付需传入'MD5'
                    paySign: data.body.payData.paySign, // 支付签名
